function oriViewer(spikeTimes, clu, eventTimes, window, trGroups)
...
f = figure;
set(gcf,'Position',[1272 167 560 749])
...
if myData.params.showAllTraces
    ...
    for g = 1:nGroups
        if ~iscell(trGroupLabels)
            psthSm(g,:) = mean(baSm(myData.trGroups==trGroupLabels(g),:));
        else
            psthSm(g,:) = mean(baSm(strcmp(myData.trGroups,trGroupLabels(g)),:));
        end
        if myData.params.showErrorShading
...
for g = 1:nGroups
    if ~iscell(trGroupLabels)
        ...
        theseCounts = spikeCounts(strcmp(myData.trGroups,trGroupLabels(g)));
    end
...
% retinotopy heat plot
axes(myData.plotAxes(3));
hold off;
positionTuning = [tuningCurve(4:end,2);tuningCurve(1:3,2)];
positionTuningReshaped = reshape(positionTuning,3,5);
imagesc(positionTuningReshaped)
