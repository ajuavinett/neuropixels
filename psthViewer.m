...
orig_colors = setColors;
% 1 = high AV; 2 = high visual; 3 = low AV; 4 = low visual
colors(1,:) = orig_colors(4,:); % rearrange colors for groups based on setColors script
colors(2,:) = orig_colors(2,:);
colors(3,:) = orig_colors(3,:);
colors(4,:) = orig_colors(1,:);
colors(5,:) = [0 0 0];
colors(6,:) = [.5 .5 .5];
...
if myData.params.showAllTraces
    if nGroups < 7
        for g = 1:nGroups
            ...
            psth_line = shadedErrorBar(bins,psthSm(g,:),stderr(g,:),'Color', colors(g,:));
            hold on;
        end
    else % if we're running an expt with an insane amount of things
        for g = 1:6
            plot(bins, psthSm(g,:), 'Color', colors(g,:), 'LineWidth', 2.0);
            hold on;
        end
        for g = 7:10
            plot(bins, psthSm(g,:), 'Color', colors(g-6,:), 'LineWidth', 2.0,'LineStyle','--');
            hold on;
        end
    end
...
box off;
% legend('high AV','high visual','low AV','low visual','sound','blank');
